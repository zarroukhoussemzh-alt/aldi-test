    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                log(f"Click error, trying JS fallback: {e}")
                # JS fallback: find one-button[circle] and click it directly
                await page.evaluate("""
                    () => {
                        const btn = document.querySelector('one-button[circle]');
                        if (btn) { btn.click(); return true; }
                        return false;
                    }
                """)
                log("JS fallback click sent.")
